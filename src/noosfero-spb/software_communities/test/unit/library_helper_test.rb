...
class LibraryHelperTest < ActiveSupport::TestCase
  ...
  should "return an empty list" do
    empty_list = []
    assert_equal  [], SoftwareCommunitiesPlugin::LibraryHelper.list_library(empty_list)
  end

  should "return a list with current library objects" do
    list_compare = []
    lib_table = SoftwareCommunitiesPlugin::LibraryHelper.list_library(@license_objects)
    assert_equal  list_compare.class, lib_table.class
  end

  should "have same information from the list passed as parameter" do
    list_compare = SoftwareCommunitiesPlugin::LibraryHelper.list_library(@license_objects)
  ...
  should "return false if list_database are empty or null" do
    list_compare = []
    assert_equal true, SoftwareCommunitiesPlugin::LibraryHelper.valid_list_library?(list_compare)
  end

  should "return a html text with license name equals to linux" do
    libraries = []

    library_description = SoftwareCommunitiesPlugin::Library.new
    library_description.name = "Lib"

    libraries << library_description
    lib_table = SoftwareCommunitiesPlugin::LibraryHelper.libraries_as_tables(libraries)
