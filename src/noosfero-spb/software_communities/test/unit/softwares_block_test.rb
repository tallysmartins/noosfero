...
class SoftwaresBlockTest < ActiveSupport::TestCase
  ...
  should 'declare its default title' do
    SoftwareCommunitiesPlugin::SoftwaresBlock.any_instance.stubs(:profile_count).returns(0)
    assert_not_equal ProfileListBlock.new.default_title, SoftwareCommunitiesPlugin::SoftwaresBlock.new.default_title
  end

  should 'describe itself' do
    assert_not_equal ProfileListBlock.description, SoftwareCommunitiesPlugin::SoftwaresBlock.description
  end

  should 'give empty footer on unsupported owner type' do
    block = SoftwareCommunitiesPlugin::SoftwaresBlock.new
